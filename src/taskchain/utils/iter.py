...
def parallel_map(fun, iterable, threads=2, desc='Running tasks in parallel.', total=None):
    ...
    async def _run():
        with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
            ...
            return [await output_value for output_value in progress_bar(asyncio.as_completed(futures), desc=desc, total=total)]
