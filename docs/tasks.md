...
This class has to be defined inside every task and describes how should be the task handled by TaskChain.
...
- **abstract** (`bool`) - if `True` this task isn't instantiated and is never part of a chain, useful for [inheritance](#tasks-inheritance)
- **name** (`str`) - if not defined, name is [derived](#stask-names-and-groups) from class name
- **group** (`str`)
...
Return type is important for [data persistence](#data-persistence). 

!!! Tip
    The method can have **arguments**. 
    TaskChain try to match and fill these arguments by values of parameters and input task.
...
!!! Warning
    ...
    Put all expensive operation to `run` method. 
    You can use [`@persistent` decorator]({{config.base_url}}/utils/#persistent-decorator). 


### Task names and groups

Name of the task (`str`) is either defined by `name` attribute of `Meta` or it is derived from class name 
...
The group can be defined in `Meta` class.
and if it is the **fullname of the task** is `group_name:task_name `. 
If you need more rich structure of groups, you can use `:` to separate multiple levels of groups, e.g. `group:subgroup`. 
...
- **dont_persist_default_value** - if value of parameter is same as the default, do not use it in persistence.
        This is useful for adding new parameters without recomputation of old data
...
This can be useful if [task inheritance](#tasks-inheritance) is used. 
Then `run` method can stay unchanged and only `input_tasks` can be redefined. 
...
- **DirData** - this class allows save arbitrary data to provided directory, 
        but data have to be handled inside `run` method manually. 
...
This allows to save partial results and next call of run method starts from last checkpoint 
when computation is interrupted.
...
After `run` method finishes and result value is saved to disk
...
This simplifies cases when pipeline contains tasks with similar functionality.
...
- changing `Meta` class
    - you can change input tasks and then computation will be done with different input.
      In this case, it is not possible have input tasks in `run` arguments, 
      and they can access by `self.input_tasks[0].value`. 
      This way, use of the task name, which is changing, is avoided. 
